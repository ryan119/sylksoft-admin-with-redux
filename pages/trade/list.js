...
import { deleteSP, exportSP, resetTradeCriteria, searchTrade, setTradeCriteria } from 'components/tradeMgmt/action'
import TradeSearchForm from 'components/tradeMgmt/trade-search'
...
const List = () => {
  ...
  const fetchData = useCallback((props) => {
    ...
    if (fetchId === fetchIdRef.current) {
      updateCriteria(props, dispatch, setTradeCriteria)
      dispatch(searchTrade())
      resetSelectedData()
    }
  }, [])
  ...
  const deleteSelectedIds = (url) => {
    if (selectedIds?.length > 0) {
      confirmDelete(() => {
        ...
        const ids = selectedIds.map((i) => i.spId)
        dispatch(deleteSP({ ids }, cb))
      })
    } else {
  ...
  const columns = [
    ...
    { Header: 'TYPE', accessor: 'type', Cell: (v) => <span title={v.row.values.spId}>{v.value}</span>},
    {
    ...
    { Header: 'Shipyard', accessor: 'shipYard', Cell: (v) => <Ellipsis variants='lg:w-32 w32'><span title={v.value}>{v.value}</span></Ellipsis> },
    { Header: 'Built Country', accessor: 'builtCountry' },
    { Header: 'TEU', accessor: 'teu' },
    /*{ Header: 'Dimension', accessor: 'dimension' },*/
    { Header: 'Gears', accessor: 'gears', Cell: (v) => <Ellipsis variants='lg:w-32 w32'><span title={v.value}>{v.value}</span></Ellipsis> },
    { Header: 'Sold Price (US$M)', accessor: 'soldPrice'},
    { Header: 'Date', accessor: 'soldDate', width: '200px' },
    { Header: 'Owners', accessor: 'owners', Cell: (v) => <Ellipsis variants='lg:w-32 w32'><span title={v.value}>{v.value}</span></Ellipsis> },
  ...
  const _onEdit = (data) => {
    router.push(`/ship/update/${data?.vesselId}?tab=2`)
  }

  const handleExport = () => {
    const cb = (data) => {
      saveAs(data, `soldPrice-${formatDatePatternDash(new Date)}.xlsx`)
    }
    dispatch(exportSP(cb))
  }

  const right = (
    <CanAccessController resource={router.pathname}>
      <ActionButton
        funcId='trade_mgmt_sp_export'
        onClick={handleExport}
        buttonColor='bg-teal-500'
        hoverColor='bg-teal-600'
      />
      <ActionButton
        funcId='trade_mgmt_sp_delete'
  ...
  return (
    <>
      <SectionTitle subtitle='成交價列表' />
      <Widget right={right} searchForm={<TradeSearchForm />}>
